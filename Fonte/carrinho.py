...
class Carrinho():
    ...
    def adicionar_carrinho(self):
        ...
        if self.id_carrinho:
            ...
            verificar_cliente_existe = """SELECT nome, cpf FROM cliente WHERE cpf = ?"""
            cursor.execute(verificar_cliente_existe, (self.id_carrinho,))
            resultado = cursor.fetchone() 
             
            if not resultado: 
                #se o cliente não existir
                conexao.close()
                return f"Cliente com cpf {self.id_carrinho} não encontrado"
    ...
    def visualizar_carrinho(self):
        ...
        ver = """SELECT cod_carrinho, nome, cod, preco, quantidade FROM carrinho WHERE cod_carrinho = ?"""
        cursor.execute(ver,(self.id_carrinho,))
