class Configuracoes():
    '''A classe Configuracoes é responsável por apresentar ao usuário opções úteis, tais como: a tabela com os prazos de envio conforme o cep, o ranking
de produtos na plataforma, o prazo de validade dos cupons, limite de parcelas e a apresentação da política de cancelamento.'''

    def __init__(self):
        pass
    def tabela_frete(self):
        #exibir tabela com cep e valor do frete conforme o cep digitado pelo usuario
        pass
    def top_produtos(self):
        #mostra produtos mais vendidos
        pass
    def validade_cupons(self):
        #mostra os cupons se eles já foram usados ou não
        pass
    def limite_parcelas(self):
        #exibe quantidade de parcelas que é possível dividir as compras
        pass
    def politica_cancelamento(self):
        #exibe a politica de cancelamento do sistema, explicando as regras de cancelamento de um pedido
        pass
